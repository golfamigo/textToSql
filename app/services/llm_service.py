...
from ..utils.config import ModelConfig, ModelProvider, settings
...
class LLMResponse:
    ...
    def __init__(
        ...
        token_usage: Optional[
            Dict[str, int]
        ] = None,  # Modified type hint to Optional[Dict[str, int]]
    ...
    def get_parsed_json(self) -> Dict[str, Any]:
        ...
        try:
            return typing.cast(
                Dict[str, Any], json.loads(self.content)
            )  # Added type cast here
        except json.JSONDecodeError as e:
            logger.error(f"JSON 解析錯誤: {e}")
            return {
                "error": f"無法解析 JSON: {str(e)}"
...
class OpenAIProvider(LLMProvider):
    ...
    def generate(
        ...
        try:
            ...
            return LLMResponse(content="", model=self.model_name, error=str(e))
...
class GoogleProvider(LLMProvider):
    ...
    def _setup(self):
        ...
        try:
            ...
            logger.error(
                "Google Generative AI 套件未安裝，請執行 pip install google-generativeai"
            )
            raise
    ...
    def generate(
        ...
        try:
            ...
            response = self.requests.post(
                f"{self.base_url}/generate", headers=headers, json=data
            )
...
class LLMService:
    ...
    def get_provider(self, model_name: Optional[str] = None) -> LLMProvider:
        ...
        if model_name in self.providers:
            return typing.cast(
                LLMProvider, self.providers[model_name]
        ...
        ):  # Added check to prevent abstract class instantiation
            raise ValueError(
                "Invalid provider class: LLMProvider is an abstract class"
            )
    ...
    def generate(
        ...
        try:
            ...
            return LLMResponse(
                content="", model=model_name or settings.default_model, error=str(e)
            )

    def rate_response(
        self, response: LLMResponse, score: float, reason: Optional[str] = None
    ) -> None:
        """為回應評分"""
        ...
        if model not in self.model_scores:
            self.model_scores[model] = {"total_score": 0.0, "count": 0, "scores": []}
